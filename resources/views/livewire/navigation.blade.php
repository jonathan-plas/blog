...
-->
      ...
      <!-- menu desplegable de las opciones de perfil-->
      @auth
        <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
          ...
          <!-- Profile dropdown -->
          <!-- Usamos x-data para darle funcionalidades a un sector del codigo -->
          <div class="ml-3 relative" x-data="{ open:false}">
            <div>
            <!-- le pasamos un evento a nuestro boton
            !open  en vez de true    -> si deseo que solo se quite la lista de opciones dando click en la foto y no debo poner el 
            x-on:click en la linea 86
            -->
            ...
            -->
            <!-- llamamos con x-show="open" con su respectivo valor que era false -->
            <div x-show="open" x-on:click.away="open = false" class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
              ...
              id="user-menu-item-2" onclick="event.preventDefault(); this.closest('form').submit();">Cerrar Sesion</a>
              </form>
            </div>
          </div>
        </div>
      ...
      @endauth
      <!-- Fin del menu desplegable de las opciones de perfil-->
    </div>
  </div>
